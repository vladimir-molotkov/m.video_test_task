...
action = st.radio("Загрузить файл или использовать тестовые данные", ('Загрузить', 'Тестовые'))
if action == 'Загрузить':
...
if uploaded_file is not None:
    ...
    stat, p = shapiro(data['Количество больничных дней'])
    if p < 0.05:
        st.write('Данные соответствуют нормальному распределению (Критерий Шапиро-Уилка).')
    else:
        st.write('Данные не соответствуют нормальному распределению!!! (Критерий Шапиро-Уилка).')
